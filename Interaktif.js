...
// ket: variable player & komputer saat ini tidak terlalu dibutuhkan,
...
// Event Listener untuk Mode PvP (orang vs orang)
...
function checkWin() {
  winningPattern.forEach((e) => {
    ...
    if (
      ...
      setTimeout(() => {
        alert("permainan sudsh berakhir, ikam pemenangnya: " + c1.innerHTML);
        container.classList.add("ended");
        turnBox.classList.add("ended");
        restartBtn.classList.add("habis dah permainannya");
      }, 570);
...
function checkDraw() {
  if (
    ...
    setTimeout(() => {
      alert("permainan berakhir, hasilnya imbang nah!");
      container.classList.add("ended");
      turnBox.classList.add("ended");
      restartBtn.classList.add("habis dah permainannya");
    }, 570);
...
function komputerStrategy(target) {
